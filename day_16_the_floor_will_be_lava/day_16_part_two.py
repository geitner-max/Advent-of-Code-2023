...
if __name__ == '__main__':
    ...
    rows = len(lines)
    cols = len(lines[0])
    # 1.) define all start positions
    # corner cases
    # beam_start_conf = [(0, 0, EAST), (0, 0, SOUTH), (0, rows - 1, EAST), (0, rows - 1, NORTH), (cols - 1, 0, SOUTH),
    #                    (cols - 1, 0, WEST),
    #                    (cols - 1, rows - 1, WEST), (cols - 1, rows - 1, NORTH)]
    beam_start_conf = []
    for i in range(cols):
        # first row
        beam_start_conf.append((i, 0, SOUTH))
        # last row
        beam_start_conf.append((i, rows - 1, NORTH))
    for j in range(rows):
        # first column
        beam_start_conf.append((0, j, EAST))
        # last column
        beam_start_conf.append((cols - 1, j, WEST))
    total_max = 0

    # 2.) Simulate beams for each configuration
    for beam_start in beam_start_conf:
        ...
        if total > total_max:
            total_max = total
    print("Solution Day 16 Part 2: ", total_max)
